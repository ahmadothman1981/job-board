    ...
    <div class="mb-4 grid grid-cols-2 gap-4">
        <div>
            ...
            <x-text-input type="text" placeholder="Search for a text" name="search"
             value="{{request('search')}}"  form-ref="filters"/>
        </div>
        <div>
        <div class="mb-1 font-semibold">Salary</div>
        <div class="flex space-x-2">
        <x-text-input type="text" placeholder="From" name="min_salary" value="{{request('min_salary')}}" form-ref="filters"/>
        <x-text-input type="text" placeholder="To" name="max_salary" value="{{request('max_salary')}}" form-ref="filters"/>
        </div>
        </div>
    <div>
